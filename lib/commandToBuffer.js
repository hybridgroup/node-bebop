...
var parseCommand = function() {
  var commandTypeId = arguments[0];
  var commandClassName = arguments[1];
  ...
  // e.g. "TakeOff"
  var command = commandClass.cmd.first({name: commandName});
  command.id = commandClass.cmd.indexOf(command);
  ...
  var offset = 3;
  if (command.arg && arguments.length > 3) {
    [].concat(command.arg).forEach(function(arg) {
      returnArgs.push({value: arguments[offset], type: arg.type});
      offset++;
    });
  }
