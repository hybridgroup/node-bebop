...
var types = {
  ...
  "u32": {
    method: function(data, index) {
      return data.readInt32LE(index);
    },
    length: 4
  },
  "double": {
    method: function(data, index) {
      return data.readDoubleLE(index);
    },
    length: 8
  },
  "string": {
    method: function(data, index) {
      return String.fromCharCode.apply(null, Array.prototype.slice.call(data, index));
    },
    length: 0
  },
  "enum": {
    method: function(data, index, arg) {
      return arg.enum[data.readInt16LE(index)]._name;
    },
    length: 2
  }
};
      ...
      if (event.arg instanceof Array) {
        event.arg.forEach(function (arg) {
          if (types.hasOwnProperty(arg._type)) { 
            args[arg._name] = types[arg._type].method(networkFrame.data, offset, arg);
        ...
        if (types.hasOwnProperty(event.arg._type)) { 
          args[event.arg._name] = types[event.arg._type].method(networkFrame.data, offset, event.arg);
        }
      }
    }
